    ...
    QUERIES = {
        ...
        "videos_length": """
            WITH all_video_urls AS (
                ...
                FROM events
                INNER JOIN event_tags ON event_id = events.id
                WHERE
                    kind=1063
                    AND lookup_created_at > 1759899999000000000
                    AND hidden=false
                    AND event_tag_key = 'm'
                    AND event_tag_value1 LIKE 'video/%'
            ),
            all_posts_with_video AS (
            SELECT
                all_video_urls.url,
                CAST(split_part(jsonb_path_query_first(tags, '$[*][*] ? (@ starts with "duration ")')#>> '{}', ' ', 2) AS bigint) AS duration
            FROM events
            INNER JOIN all_video_urls ON all_video_urls.url = split_part(jsonb_path_query_first(tags, '$[*][*] ? (@ starts with "url ")')#>> '{}', ' ', 2) AND all_video_urls.url IS NOT NULL
            WHERE
                kind IN (1, 30175, 30023)
                AND hidden=false
                AND lookup_created_at > 1759899999000000000
                AND jsonb_path_query_first(tags, '$[*][*] ? (@ starts with "duration ")') IS NOT NULL
            ),
            uniq_video AS (
                SELECT url, duration FROM all_posts_with_video GROUP BY url, duration
            )
            SELECT SUM(duration)::bigint FROM uniq_video;
        """,
        ...
        try:
            ...
            with psycopg.connect(conn_string) as conn:
                with conn.cursor() as cur:
                    ...
                    cur.execute(cls.QUERIES["videos_length"])
                    results["videos_length"] = cur.fetchone()[0]
