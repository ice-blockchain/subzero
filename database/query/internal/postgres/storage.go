...
func MustConnect(ctx context.Context, cfg *Config, ddl string) *DB {
	if cfg.Storage.PrimaryURL != "" || len(cfg.Storage.ReplicaURLs) > 0 {
		globalDB = mustConnectWithCfg(context.Background(), &cfg.Storage, "")
	}
	if globalDB != nil && !cfg.Storage.IgnoreGlobal {
	...
	return mustConnectWithCfg(ctx, &cfg.Storage, ddl)
}
...
func mustRunDDL(ctx context.Context, master *pgxpool.Pool, ddl string) {
	for _, statement := range strings.Split(ddl, "----") {
		...
		if err != nil {
			panic(fmt.Sprintf("subkeys fetch failed: %v", err))
		}
	}
}
...
func mustConnectPool(ctx context.Context, timeout, user, pass, url string) (db *pgxpool.Pool) {
	...
	log.Print(fmt.Sprintf("poolConfig.MaxConnIdleTime=%v", poolConfig.MaxConnIdleTime))
	if !strings.Contains(strings.ToLower(url), "pool_health_check_period") {
		poolConfig.HealthCheckPeriod = 30 * stdlibtime.Second
	}
	log.Print(fmt.Sprintf("poolConfig.HealthCheckPeriod=%v", poolConfig.HealthCheckPeriod))
...
func doAfterConnect(ctx context.Context, timeout string, conn *pgx.Conn) error { //nolint:funlen // .
	...
	log.Print(fmt.Sprintf("storage:timeout:%v", timeout))
...
func retry[T any](ctx context.Context, op func() (T, error)) (tt T, err error) {
	err = backoff.RetryNotify(
		...
		func(e error, next stdlibtime.Duration) {
			log.Print(errors.Wrapf(e, "[storage]call failed. retrying in %v... ", next))
		})
