...
func ValidateAuthEvent(event *Event, challenge string, relayURL string) (pubkey string, err error) {
	...
	if event.Kind != nostr.KindClientAuthentication {
		return "", errors.Wrapf(ErrWrongEventParams, "event kind %d: expected %v", event.Kind, nostr.KindClientAuthentication)
	}

	if event.Tags.GetFirst([]string{"challenge", challenge}) == nil {
		return "", errors.Wrapf(ErrWrongEventParams, "event does not contain challenge tag or it does not match")
	}
	...
	if dest == nil {
		return "", errors.Wrap(ErrWrongEventParams, "event does not contain relay tag")
	}
	...
	if expected.Scheme != found.Scheme ||
		...
		return "", errors.Wrapf(ErrWrongEventParams, "relay URL mismatch: expected %v, found %v", expected.String(), found.String())
	}

	now := time.Now()
	eventTime := event.CreatedAt.Time()
	if eventTime.Before(now.Add(-leeway)) || eventTime.After(now.Add(leeway)) {
		return "", errors.Wrapf(ErrWrongEventParams, "event time is out of bounds: %v", eventTime)
	}

	if ok, err := event.CheckSignature(); err != nil {
		return "", errors.Wrap(err, "failed to check event signature")
	} else if !ok {
		return "", errors.Wrap(ErrWrongEventParams, "event signature is invalid")
	}
