...
func TestNIP96(t *testing.T) {
	...
	helperCreateAttestations(t, appcontext.TestContext(t), master, masterPubKey, user1PubKey, user2PubKey)
	...
	t.Run("delete file on same relay", func(t *testing.T) {
		...
		wg.Go(func() {
			status := deleteFile(t, appcontext.TestContext(t), user1, fileHash, masterPubKey)
	...
	t.Run("Second node", func(t *testing.T) {
		...
		t.Run("delete file by same hash used in multiple posts does not break link", func(t *testing.T) {
			...
			deleteFileAndVerify(func(fileName string) {
				...
				status, location := download(t, appcontext.TestContext(t), user1, "c7fce3cad585a3110c96b34516df16362c99f6f32359d64ddf1a58c1710247d1", masterPubKey)
		...
		t.Run("download endpoint redirects to same download url over ton storage", func(t *testing.T) {
			...
			status, location := download(t, appcontext.TestContext(t), user1, "777d453395088530ce8de776fe54c3e5ace548381007b743e067844858962218", masterPubKey)
			require.Equal(t, http.StatusFound, status)
			...
			status, location = download(t, appcontext.TestContext(t), user1, "b2b8cf9202b45dad7e137516bcf44b915ce30b39c3b294629a9b6b8fa1585292", masterPubKey)
			require.Equal(t, http.StatusFound, status)
			require.Regexp(t, fmt.Sprintf("^http://[0-9a-fA-F]{64}.bag/%v", expected.Summary), location)
		...
		t.Run("list files with pagination", func(t *testing.T) {
			files := list(t, appcontext.TestContext(t), user1, 0, 1, masterPubKey)
			require.Equal(t, uint32(filesCount-1), files.Total)
			...
			files = list(t, appcontext.TestContext(t), user1, 1, 1, masterPubKey)
			require.Equal(t, uint32(filesCount-1), files.Total)
			require.Len(t, files.Files, 1)
			for _, f := range files.Files {
				verifyFile(t, f.Content, f.Tags)
		...
		t.Run("delete file owned by user 1 on behave of usr 2 (attestation) via deletion of imeta tagged post", func(t *testing.T) {
			...
			status := deleteFile(t, appcontext.TestContext(t), user2, "982d9e3eb996f559e633f4d194def3761d909f5a3b647d1a851fead67c32c9d1", masterPubKey)
			require.Equal(t, http.StatusOK, status)
		...
		t.Run("delete file owned by master by usr1 (Forbidden)", func(t *testing.T) {
			status := deleteFile(t, appcontext.TestContext(t), user1, "fc613b4dfd6736a7bd268c8a0e74ed0d1c04a959f59dd74ef2874983fd443fc9", masterPubKey)
