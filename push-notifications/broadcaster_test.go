...
func helperWaitForNotifications(t *testing.T, mockClient *mockNotificationClient, notificationType NotificationType, expectedEvent *model.Event) {
	...
	t.Logf("Waiting for push notification of type %s for event %s", notificationType, expectedEvent.ID)
	select {
	...
	case <-time.After(time.Second * 5):
		require.FailNow(t, "Timed out waiting for push notification")
	}
}
...
func TestNotificationBroadcastEndToEnd(t *testing.T) {
	...
	t.Run("Publish EditableTextNote from user1", func(t *testing.T) {
		...
		for i := range user3.Relays {
			t.Logf("Waiting for broadcast to User3's relay %s", user3.Relays[i])
			select {
			...
			case <-time.After(time.Second * 5):
				require.FailNow(t, "Timed out waiting for broadcasts to User3's relays")
			}
		}
	...
	t.Run("Publish_TokenizedCommunityDefinition_From_User2", func(t *testing.T) {
		...
		// Wait just for a single event since we have deduplication inside RQ.
		helperWaitForNotifications(t, mockNotificationClient, NotificationTypeTokenizedCommunityCreated, postEvent)
	})
}
