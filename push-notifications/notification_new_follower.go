...
func (pm *PushNotificationManager) handleNewFollowerEvent(ctx context.Context, event *model.Event, relevantEvents ...*model.Event) ([]*pn.Notification[*DeviceRegistrationEvent], error) {
	...
	return pm.createNewFollowerNotification(event, recipientPubKey, relevantEvents...), nil
}
...
func (pm *PushNotificationManager) createNewFollowerNotification(event *model.Event, recipientPubKey string, relevantEvents ...*model.Event) []*pn.Notification[*DeviceRegistrationEvent] {
	devices := pm.collectUserValidDevices(recipientPubKey, event)

	return pm.createNotifications(devices, NotificationTypeNewFollower, event, relevantEvents...)
}
