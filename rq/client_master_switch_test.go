...
var (
	testContainer *query.Container
)

func TestMain(m *testing.M) {
	ctx, cancel := appcontext.NewAppContext(context.Background())
	testContainer = query.NewTestContainer(ctx)
...
func TestDatabaseClientMasterSwitch(t *testing.T) {
	...
	t.Run("SwitchMasterOnConnectionFailure", func(t *testing.T) {
		client, err := newClient(ctx,
			WithConfig(&Config{
				Config: query.Config{
					WriteURLs: []string{proxyAddr1, proxyAddr2},
		...
		require.NoError(t, client.HealthCheck(ctx), "initial health check should pass")
		...
		err = client.DB.switchMaster(ctx, io.EOF)
		...
		require.NoError(t, client.initRiverClient(ctx, client.DB.Get()), "should reinitialize river client")

		// Verify new connection works.
		require.NoError(t, client.HealthCheck(ctx), "health check should pass after switch")
	})
}
...
func TestDatabaseClientSwitchBackOnRecovery(t *testing.T) {
	...
	require.NoError(t, client.initRiverClient(ctx, client.DB.Get()))
	require.NoError(t, client.HealthCheck(ctx), "health check should pass after first switch")
	...
	require.NoError(t, client.initRiverClient(ctx, client.DB.Get()))
	require.NoError(t, client.HealthCheck(ctx), "health check should pass after switching back")
}
...
func TestPushWithMasterSwitch(t *testing.T) {
	...
	require.NoError(t, client.Push(ctx, args1))
