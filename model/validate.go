...
var (
	...
	CommongTags                 = tagsTable("nonce", "expiration", "imeta", CustomIONTagOnBehalfOf, "settings")
	KindSupportedTags           = map[Kind]map[string]struct{}{
		nostr.KindProfileMetadata:       tagsTable("e", "p", "a", "alt"),
		nostr.KindTextNote:              tagsTable("e", "p", "q", "l", "L", "h"),
		...
		nostr.KindArticle:               tagsTable("a", "d", "e", "t", "title", "image", "summary", "published_at", "h"),
		...
		// Community
		KindCommunityDefinition:            tagsTable("h", "name", "description", "public", "private", "open", "closed", "p", "a"),
		KindCommunityOwnershipTransferring: tagsTable("h", "a", "p"),
		KindCommunityJoin:                  tagsTable("h", "p", "authorization"),
		KindCommunityBanUser:               tagsTable("h", "p"),
		KindCommunityChangeDefinition:      tagsTable("h", "name", "description", "public", "private", "open", "closed", "p"),
	}
...
func Validate(ctx context.Context, e *Event) error {
...
func validateKindTextNoteEvent(ctx context.Context, e *Event) error {
	if json.Valid([]byte(e.Content)) {
...
func validateKindRepostEvent(ctx context.Context, e *Event) error {
...
func validateKindCommunityDefinitionEvent(ctx context.Context, e *Event) error {
	var (
		hTag       = e.GetTag("h")
		pTags      = e.Tags.GetAll([]string{"p"})
	...
	for _, tag := range pTags {
		if tag.Key() == "p" {
			if len(tag) < 4 || (tag[3] != string(ModeratorRole) && tag[3] != string(AdminRole) && tag[3] != "") {
	...
	for _, aTag := range aTags {
		...
		if splitted := strings.Split(aTag.Value(), ":"); len(splitted) != 3 || splitted[0] != fmt.Sprint(KindCommunityDefinition) {
			return errors.Wrapf(ErrWrongEventParams, "community ownership must have a valid a tag: %+v", e)
		}
	}
	if e.Kind == KindCommunityDefinition {
		return nil
	}
	...
	authorRole := GetCommunityRoleByPubkey(e.GetMasterPublicKey(), communityDefinitionEvent)
	if authorRole == OthersRole {
		...
		for _, pTag := range pTags {
			if pTag.Key() == "p" {
				...
				for _, tag := range communityDefinitionEvent.Tags.GetAll([]string{"p"}) {
					if tag.Key() == "p" && tag.Value() == pTag.Value() && Role(tag[3]) == AdminRole {
...
func validateKindCommunityJoinEvent(ctx context.Context, e *Event) error {
	var (
		hTag             = e.GetTag("h")
	...
	if authorizationTag != nil {
		...
		if parsedContent.Kind != KindCommunityJoin {
	...
	if closedTag := communityDefinitionEvent.GetTag("closed"); closedTag != nil {
		if authorRole := GetCommunityRoleByPubkey(e.GetMasterPublicKey(), communityDefinitionEvent); authorRole != OthersRole {
			return nil
		}
		if authorizationTag == nil {
			return errors.Wrap(ErrCommunityActionForbidden, "can't join closed community")
		}
		var parsedAuthorizationEvent Event
		...
		if authorizationRole := GetCommunityRoleByPubkey(parsedAuthorizationEvent.GetMasterPublicKey(), communityDefinitionEvent); authorizationRole == OthersRole {
...
func validateKindCommunityOwnershipTransferringEvent(ctx context.Context, e *Event) error {
	var (
		aTags         = e.Tags.GetAll([]string{"a"})
		hTag          = e.GetTag("h")
		pTags         = e.Tags.GetAll([]string{"p"})
	...
	if authorRole := GetCommunityRoleByPubkey(e.GetMasterPublicKey(), communityDefinitionEvent); authorRole != OwnerRole {
...
func validateKindCommunityBanUserEvent(ctx context.Context, e *Event) error {
	var (
		hTag  = e.GetTag("h")
		pTags = e.Tags.GetAll([]string{"p"})
	)
	...
	for _, pTag := range pTags {
		if pTag.Key() == "p" {
			...
			toBanRole := GetCommunityRoleByPubkey(pTag.Value(), communityDefinitionEvent)
			if toBanRole == AdminRole && authorRole == ModeratorRole {
...
func validateSettingsTag(kind int, tag nostr.Tag) error {
	...
	switch settingType {
	case CommentsEnabledSettings:
		if kind != KindCommunityDefinition && kind != KindCommunityChangeDefinition {
