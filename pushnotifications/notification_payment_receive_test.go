...
func TestHandlePaymentReceiveNotification(t *testing.T) {
	t.Parallel()
	...
	notifications := pm.handlePaymentReceiveNotification(&event.Event, "en")
	...
	require.Equal(t, "Payment Received", notification.Title, "Title should match")
	require.Equal(t, "You've sent a payment", notification.Body, "Body should match")
